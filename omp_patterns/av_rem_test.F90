...
contains
  ...
  ! ------------------------------------------------------------------ !
  !  Comparison helper for 2-D arrays (assumed-shape).                !
  !  NOTE: when av_rem_v(0:nx+1,ny) is passed, size(a,1)=nx+2 so     !
  !  the first-diff i index is 1-based (physical i = displayed_i - 1).!
  ! ------------------------------------------------------------------ !
  subroutine compare_2d(label, a, b, passed)
    ...
    if (passed) then
      ...
      outer: do j = 1, nj ; do i = 1, ni
        if (a(i,j) /= b(i,j)) then
          write(*,'(A,2I4)')       '    first diff (i,j)=', i-1, j   ! i is 1-based here; physical i = i-1
